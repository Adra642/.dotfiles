    ...
    while True:
        user_input = (
            input(
                ...
                + "Warning: "
                + "\033[0m"
                + "The script will close Zellij if it is running. Do you want to continue? (y/n): "
            )
            .strip()
            .lower()
        )
...
def select_scheme(schemes):
    ...
    while True:
        current_scheme = input(
            "\033[94m"
            + "Please enter the color scheme number or 'x' to exit: "
            + "\033[0m"
        )
        ...
        try:
            ...
            if 0 <= current_scheme < len(schemes):
                ...
                print(
                    f"\033[31mInvalid input. The number must be between 0 and {len(schemes) - 1}.\033[0m"
                )
        except ValueError:
